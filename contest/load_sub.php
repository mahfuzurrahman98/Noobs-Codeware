...
for ($i = 0; $i < count($probs); $i++) {
    $qry = "SELECT Title FROM Problem WHERE Id = '{$probs[$i]}';";
    $res = mysqli_query($con, $qry);
...
if ($username == 'Friends Only') {
    $qry = "SELECT * FROM Contest_Submission WHERE Contest_Id = '{$id}' AND Problem LIKE '{$prb}' AND Verdict LIKE '%{$verdict}%' AND Language LIKE '%{$language}' AND Username IN (SELECT Followed FROM Friend WHERE _user = '{$unm}')";
}
else {
    $qry = "SELECT * FROM Contest_Submission WHERE Contest_Id = '{$id}' AND Username LIKE '%{$username}%' AND Problem LIKE '{$prb}' AND Verdict LIKE '%{$verdict}%' AND Language LIKE '%{$language}'";
}
