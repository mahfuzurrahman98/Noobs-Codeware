...
if ($username == 'Friends Only') {
    $qry = "SELECT * FROM User_Contest_Submission WHERE Contest_Id = '{$id}' AND Problem LIKE '%{$title}%' AND Verdict LIKE '%{$verdict}%' AND Language LIKE '%{$language}%' AND Username IN (SELECT Followed FROM Friend WHERE _user = '{$unm}')";
}
else {
    $qry = "SELECT * FROM User_Contest_Submission WHERE Contest_Id = '{$id}' AND Username LIKE '%{$username}%' AND Problem LIKE '%{$title}%' AND Verdict LIKE '%{$verdict}%' AND Language LIKE '%{$language}%'";
}
$qry .= " ORDER BY Sub_Time DESC LIMIT {$lim} OFFSET {$off};";
$res = mysqli_query($con, $qry) or die('Error');

while ($row = mysqli_fetch_assoc($res)) {
    $qry_pid = "SELECT Id FROM User_Problem WHERE Title = '{$title}';";
    ...
    <td>
        <?php if ($_SESSION['Username'] == $row['Username']) { ?>
            <a class="code_modal" href="javascript:void(0)" data-toggle="modal" data-target="#show_code" data-id=<?php echo $row['Sub_Id']; ?>><?php echo sprintf('%04d',$row['Sub_Id']); ?></a>
        <?php } else { ?>
            <?php echo sprintf('%04d',$row['Sub_Id']); ?>
        <?php } ?>
    </td>
