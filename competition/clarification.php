...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="mx-lg-5 mb-5">
            <table class="table table-success table-hover rounded" style="cursor: pointer; cursor: hand;">
                <tbody>
                <?php
                $qry = "SELECT Cmnt_Id, Ques FROM User_Clarifi_Comment WHERE Contest_Id = '{$id}' AND _user = 'judge';";
                $res = mysqli_query($con, $qry);
                while ($row = mysqli_fetch_assoc($res)) {
                    $Cmnt_Id = $row['Cmnt_Id'];
                    $Ques = $row['Ques']; ?>
    ...
    if (isset($_POST['ask_ques_submit'])) {
        ...
        $qry = "INSERT INTO User_Clarifi_Comment(Contest_Id, _user, Problem, Ques) VALUES('{$id}', '{$unm}', '{$ask_prb}', '{$ask_ques}');";
