...
$id = $_GET['id'];
$flag = TRUE;
$qry = "SELECT Title, Description, Rules, Problems, Start_Time, Finish_Time, Judges FROM User_Contest WHERE Id = '{$id}';";
...
while ($row = mysqli_fetch_assoc($res)) {
    ...
    $jud_arr = explode('/', substr(substr($row['Judges'], 1), 0, -1));
}
$prb_num = 'A';
...
<body>
    ...
    <div class="container-fluid">
        ...
        <nav class="my-5 prb_info_tab fub mx-3 mx-lg-5">
            <div class="row px-3 py-1 shadow-lg bg-light rounded text-center justify-content-center">
        ...
        <div class="my-5">
            <h3>Judges</h3>
            <?php
            foreach ($jud_arr as $judg) {
                $judg = trim($judg);
                $qry = "SELECT Username FROM User WHERE Username = '{$judg}';";
                $res = mysqli_query($con, $qry);
                if (mysqli_num_rows($res) > 0) {
                    $judg = '<a href="../profile.php?user='.$judg.'">'.$judg.'</a>';
                }
                echo '<span class="mx-1 shadow shadow-lg bg-light rounded p-1">'.$judg.'</span>';
            }
            ?>
        </div>
