...
<head>
    ...
    <title>Testcases - <?php echo $tit; ?></title>
...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="mt-5 mx-lg-5 text-center">
            <p class="alert alert-warning text-left">You are advised to provide a solution already written. You can still avoid it, but since we assume that you created the test case by running the source code, it's best to provide a judge's solution. Use our <a href="../../codeground">native code editor</a> to test and run code.</p>
        ...
        <div class="shadow-lg bg-white rounded mt-5 p-3 mx-lg-5 px-lg-5">
            <?php
          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-body">
                <div class="row">
                    <div class="col-11"><h3 class="text-center"><b>Add a new testcase</b></h3></div>
                ...
                <form action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
                    <div class="form-group">
                        ...
                        <textarea name="tc_inp" class="form-control mb-2" rows="7" required></textarea>
                    </div>
                    <div class="form-group">
                        <label><b>Write the output:</b></label>
                        <textarea name="tc_out" class="form-control mb-2" rows="7" required></textarea>
                    </div>
                    <button type="submit" name="addtst" class="btn btn-success"><b>Add</b></button>
                </form>
              </div>
            </div>
          </div>
        ...
        if(isset($_POST['addtst'])) {
            ...
            $dir = '../../uploads/admin/testcases/'.$pid.'/input';
            $last_file = '' . get_last($dir) + 1;

            shell_exec('echo> ../../uploads/admin/testcases/'.$pid.'/input/'.$last_file.'.txt');
            shell_exec('echo> ../../uploads/admin/testcases/'.$pid.'/output/'.$last_file.'.txt');
            file_put_contents('../../uploads/admin/testcases/'.$pid.'/input/'.$last_file.'.txt', $tc_inp);
            file_put_contents('../../uploads/admin/testcases/'.$pid.'/output/'.$last_file.'.txt', $tc_out);
            echo '<script>
