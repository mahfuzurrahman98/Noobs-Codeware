...
foreach ($arr as $cur_sub) {
	...
	if ($verdict == 'Accepted' && !isset($done[$user.','.$problem])) {
		...
		if (!isset($first_solved_tym[$user])) {$first_solved_tym[$user] = $subtym;}
		if (!isset($total_scored[$user])) {$total_scored[$user] = 0;}
		if (!isset($total_penalty[$user])) {$total_penalty[$user] = 0;}
...
function cmp($a, $b) {
	...
	if ($total_scored[$a] == $total_scored[$b]) {
		if ($total_penalty[$a] == $total_penalty[$b]) {
